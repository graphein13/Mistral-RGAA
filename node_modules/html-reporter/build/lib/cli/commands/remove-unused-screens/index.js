...
async function handleScreens(allScreenPaths, screensToRemove, { spinner, cliOpts } = {}) {
    const existentScreenPaths = await Promise.filter(screensToRemove.paths, async (screenPath) => {
        try {
            ...
            if (err.code === 'ENOENT') {
                logger.warn(chalk.red(`Screen by path: "${screenPath}" is not found in your file system. ` +
    ...
    logger.log(`Found ${chalk[screenPathsToRemoveLen > 0 ? 'red' : 'green'](screenPathsToRemoveLen)} ` +
